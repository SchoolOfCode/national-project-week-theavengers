...
function Login() {
    ...
    async function handleLogin(formInfo){
        ...
        if(password != undefined) {
            ...
            setResMsg('Please enter a password')
        }
     ...
     <Container>
        ...
        <Row>
            ...
            <h6>{resMsg}</h6>
                            ...
                            <Button className="login-input" variant="primary" onClick={()=> handleLogin(latest.current.email, latest.current.password)}><NavLink style={{color: 'white', textDecoration:'none'}}to={'/Home'}>Login</NavLink></Button>
            </Modal.Body>
